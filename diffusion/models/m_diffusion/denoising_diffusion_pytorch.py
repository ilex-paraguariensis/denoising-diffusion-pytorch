...
class Unet(nn.Module):
    def __init__(
        ...
        self.final_conv = nn.Sequential(nn.Conv2d(dim, 10, 1))
...
class GaussianDiffusion(nn.Module):
    ...
    def predict(self, x):
        ...
        t = torch.tensor(self.num_timesteps, device=x.device, dtype=torch.long).repeat(x.shape[0])
        # ipdb.set_trace()
        return self.model(noise, t, cond=x)
